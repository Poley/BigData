...
public class Query extends Configured implements Tool {
	public int run(String[] args) throws Exception {
		...
		conf.set("mapred.jar", "file:////users/level4/0801466p/Level4/BigData/Ex2/Query5/MyJar.jar");
		
		conf.set("time", args[0]);		
		...
		if (job.waitForCompletion(true)){
			...
			for (Result res : scanner) {
				...
				byte[] rev = res.getValue(Bytes.toBytes("q5"),Bytes.toBytes("rev"));
				byte[] time = res.getValue(Bytes.toBytes("q5"),Bytes.toBytes("time"));
				//if ( Bytes.toInt(sum)>1)
					System.out.println(Bytes.toLong(artid) + " " + Bytes.toLong(rev) + " " + Bytes.toString(time) + "\n");
				if (--rows == 0)
					break;
